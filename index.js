...
background.src = "https://i.ibb.co/64wxXVY/background1.png";
...
enemyImage.src = '/assets/__cartoon_fish_06_black_swim.png';
...
killerBackground.src = '/assets/killer.png'
...
class Killer {
  ...
  update() {
    this.x += this.speed;
    if (this.x > canvas.width + this.spriteWidth) {
      this.x = -200;
      this.y = Math.random() * (canvas.height - 150) + 90;
      this.speed = Math.random() * 2 + 2;
    }
...
function handleGameOver() {
  ctx.fillStyle = 'white';
  ctx.fillText("GAME OVER, you reached score: " + score, canvas.width / 2 - canvas.width / 5, canvas.height / 2);
  gameOver = true;
}
